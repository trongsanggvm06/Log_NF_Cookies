...
// ===== CHECK ACC MODE =====
const checkaccForm = document.getElementById("checkacc-form");
const checkaccInput = document.getElementById("checkacc-input");
const checkaccBtn = document.getElementById("checkacc-btn");
const checkaccSpinner = document.getElementById("checkacc-spinner");
const checkaccResults = document.getElementById("checkacc-results");

document.getElementById("clear-checkacc").addEventListener("click", () => {
  checkaccInput.value = "";
  checkaccResults.innerHTML = '<div class="empty-state"><div class="icon">🔍</div>Thông tin tài khoản sẽ hiện ở đây</div>';
});
...
function buildAccountCard(acc) {
  ...
  add("👤 Name", acc.name);
  add("📧 Email", acc.email);
  add("🌍 Country", `${acc.country || ""} ${acc.countryFlag || ""}`.trim());
  add("📦 Plan", acc.plan);
  add("📅 Member Since", acc.memberSince);
  if (isHit) {
    add("🗓️ Next Billing", acc.nextBilling);
    add("💳 Payment", acc.payment);
    add("💳 Card", acc.maskedCard);
    add("📱 Phone", acc.phone);
    add("🎞️ Quality", acc.quality);
    add("📺 Streams", acc.maxStreams);
    add("💰 Price", acc.price);
    add("⏸️ Hold Status", acc.holdStatus);
    add("👥 Extra Member", acc.extraMember);
  }
  add("✅ Email Verified", acc.emailVerified);
  add("🛡️ Membership", acc.membershipStatus);
  const profiles = acc.profiles || [];
  add(`🎭 Profiles (${profiles.length})`, profiles.length ? profiles.join(", ") : null);

  const rowStyle = "display:flex;justify-content:space-between;gap:12px;padding:7px 0;border-bottom:1px solid rgba(255,255,255,.06);font-size:.85rem;";
  const labelStyle = "color:var(--text-muted);white-space:nowrap;";
  const valueStyle = "color:var(--text);text-align:right;word-break:break-word;";
  const rowsHtml = rows.map(([l, v]) =>
    `<div style="${rowStyle}"><span style="${labelStyle}">${escHtml(l)}</span><span style="${valueStyle}">${escHtml(v)}</span></div>`
  ).join("");

  card.innerHTML = `
    <div class="result-header">
      <span class="badge ${isHit ? "badge-ok" : "badge-fail"}">${isHit ? "✅ HIT" : "🆓 FREE"}</span>
      <span class="result-title">${escHtml(acc.plan || "")}</span>
    </div>
...
checkaccForm.addEventListener("submit", async (e) => {
  e.preventDefault();
  const raw = checkaccInput.value.trim();
  if (!raw) return;

  checkaccBtn.disabled = true;
  checkaccSpinner.style.display = "inline-block";
  checkaccResults.innerHTML = "";

  try {
    const resp = await fetch("/api/checkacc", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ cookies: raw }),
    });
    ...
    if (data.ok && data.account) {
      checkaccResults.appendChild(buildAccountCard(data.account));
    } else {
      checkaccResults.innerHTML = `<div class="result-card error-card"><div class="error-msg">❌ ${escHtml(data.error || "Không lấy được thông tin tài khoản")}</div></div>`;
    }
  } catch {
    checkaccResults.innerHTML = '<div class="result-card error-card"><div class="error-msg">❌ Lỗi kết nối đến server</div></div>';
  } finally {
    checkaccBtn.disabled = false;
    checkaccSpinner.style.display = "none";
  }
});
